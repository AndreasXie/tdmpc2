...
class Buffer_atari_faster():
	...
	def _prepare_batch_atari(self, td):
		...
		if self.cfg.gray_scale:
			obs = obs.unfold(0, self.cfg.frame_stack, 1).squeeze(2).permute(0, 1, 4, 2, 3)
		else:
			obs = obs.unfold(0, self.cfg.frame_stack, 1).permute(0, 1, 2, 5, 3, 4) 
			obs = obs.reshape(-1,self.cfg.batch_size, 12, 84, 84)
