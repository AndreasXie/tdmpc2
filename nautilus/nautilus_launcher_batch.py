

import os
import sys
import re
import itertools
import tempfile
import termcolor
import string
import random
import codecs
import subprocess
from pathlib import Path
import time
import yaml


def _get_current_context():
	cmd = ["kubectl", "config", "current-context"]
	output = subprocess.check_output(cmd)
	return output.decode().strip()


def _safe_str(name, length=8):
	name = re.sub(r'(\W)\1+', r'\1', re.sub(r'[^a-zA-Z0-9]', '-', name))
	if len(name) > length:
		name = name[:length]
	else:
		name += 'a' * (length - len(name))
	return name.lower()


def _encode_name(name):
	uid = ''.join(random.choice(string.ascii_lowercase) for _ in range(4))
	return codecs.encode((f'{_safe_str(name)}-{uid}').replace('-', ''), 'rot13')


def _read_text(fp):
	with open(fp, 'r',encoding='utf-8') as f:
		text = f.read()
	return text


def _submit(args, name, seeds=None):
	context = _get_current_context()
	template = _read_text('nautilus/job.yaml')
	wandb_key = _read_text('nautilus/wandb.key')
	cfg = yaml.safe_load(Path(f'{os.getcwd()}/nautilus/config.yaml').read_text(encoding='utf-8'))
	cmd = cfg.get('cmd', None)
	assert cmd is not None, f'No cmd found in config:\n{cfg}'
	cfg.update(
		name='yx-'+_encode_name(name),
		namespace=context,
		wandb_key=wandb_key
	)
	if seeds is not None:
		base_cmd = cmd.split('&')[0].split('nohup')[1].strip()
		_cmd_lines = []
		for i, seed in enumerate(seeds):
			_cmd = f"nohup {base_cmd} seed={seed} {args} > /dev/null 2>&1 &"
			
			if i != 0:
				_cmd = " "*12 + _cmd
			_cmd_lines.append(_cmd)
				

		# 将命令行拼接为多行字符串
		_cmd = "\n".join(_cmd_lines)

		_cmd = "\n".join(line for line in _cmd.splitlines())
	
		cfg.update(cmd=_cmd)

	while '{{' in template:
		for key, value in cfg.items():
			regexp = r'\{\{\s*' + str(key) + r'\s*\}\}'
			template = re.sub(regexp, str(value), template)
	tmp = tempfile.NamedTemporaryFile(suffix='.yaml')
	tmp_name = tmp.name.split("\\")[-1]
	tmp_path = Path(f'{os.getcwd()}/{tmp_name}')

	with open(tmp_path, 'w') as f:
		f.write(template)
	print(termcolor.colored(f'{cfg["name"]}', 'yellow'), args if len(args) > 0 else 'None')
	tmp.flush()
	tmp.close()
	# if 'seed' in template:
	# 	print(tmp_path)
	# 	print(termcolor.colored(f'Warning: seed is not supported', 'red'))
	# 	return 
	os.system(f'./kubectl create -f {tmp_path}')
	print(termcolor.colored(f'Job submitted', 'green'))


def _submit_batch(kwargs: dict):
	if 'seed' in kwargs and isinstance(kwargs['seed'], list):
		seeds = kwargs['seed']
		del kwargs['seed']
	else:
		seeds = None
	arg_list = list(itertools.product(*kwargs.values()))
	if len(arg_list) > 32:
		print(termcolor.colored(f'Error: {len(arg_list)} jobs exceeds limit of 32', 'red'))
		return
	print(termcolor.colored(f'Submitting {len(arg_list)} job{"s" if len(arg_list) > 1 else ""}', 'green'))
	# if seeds is not None:
	# 	kwargs['seed'] = seeds
	for args in arg_list:
		print(arg_list)
		# if seed is not None:
		#  	args = (*args, seed)
		args = ' '.join([f'{k}={v}' if k != 'seed' else '' for k, v in zip(kwargs.keys(), args)])
		_submit(args, name=kwargs['exp_name'][0] if 'exp_name' in kwargs else 'default', seeds = seeds if seeds else None)

def launch():
	kwargs = dict(arg.split('=') for arg in sys.argv[1:])
	kwargs = {k: v.split(',') for k, v in kwargs.items()}
	print(kwargs)
	_submit_batch(kwargs)

if __name__ == "__main__":
    import sys

    # Define the test parameters
    test_args = ['exp_name=Asterix_mpc', 'mpc=true', 'seed=1,3']

    # Backup the original sys.argv
    original_argv = sys.argv.copy()

    try:
        # Set sys.argv to the test arguments
        sys.argv = test_args

        # Call the launch function with the test parameters
        launch()
    finally:
        # Restore the original sys.argv
        sys.argv = original_argv